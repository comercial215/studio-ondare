...
interface EquipeProps {
  /** null = time interno do Studio Ondare (visível em todos os quadros); uuid = contatos daquele cliente */
  workspaceId: string | null;
  legenda?: string;
  compacto?: boolean;
}

export default function Equipe({ workspaceId, legenda, compacto }: EquipeProps) {
  ...
  async function adicionar(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from("team_members")
      .insert({ nome: nome.trim(), email: email.trim(), cargo: cargo.trim() || null, workspace_id: workspaceId });
    if (error) {
  ...
  async function remover(membro: TeamMember) {
    if (!confirm(`Remover ${membro.nome}? As tarefas já atribuídas a essa pessoa continuam existindo, só ficam sem responsável.`))
      return;
    ...
    <div>
      ...
      <form onSubmit={adicionar} className="glass mb-4 flex flex-wrap items-end gap-2 rounded-xl p-4">
        <div className="flex-1 min-w-[140px]">
          ...
          <input value={nome} onChange={(e) => setNome(e.target.value)} className="equipe-input" required />
        </div>
        <div className="flex-1 min-w-[160px]">
          <label className="mb-1 block text-xs font-medium text-foreground/80">E-mail</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="equipe-input" required />
        </div>
        <div className="flex-1 min-w-[120px]">
          <label className="mb-1 block text-xs font-medium text-foreground/80">Cargo</label>
          <input value={cargo} onChange={(e) => setCargo(e.target.value)} className="equipe-input" />
        </div>
      ...
      <div className={compacto ? "overflow-hidden rounded-xl border border-border" : "glass overflow-hidden rounded-xl"}>
        {carregando ? (
  ...
  return (
    <tr className="border-t border-border">
      <td className="px-4 py-1.5">
        <button type="button" onClick={() => inputFoto.current?.click()} className="relative block" title="Trocar foto">
      ...
      <td className="px-4 py-1.5 text-right">
        <button onClick={() => onRemover(membro)} className="rounded px-2 py-1 text-xs text-muted hover:bg-alert-bg hover:text-alert">
          Remover
        </button>
      </td>
    </tr>
